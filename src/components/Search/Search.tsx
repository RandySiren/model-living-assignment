...
const Search = ({ setSearchResults }: SearchProps): JSX.Element => {
  ...
  const handleClick = (e: React.MouseEvent<HTMLButtonElement, MouseEvent>) => {
    ...
    fetch(
      `http://api.giphy.com/v1/gifs/search?api_key=jdNWJ6plFGt0nIza8kMG7znyZgVK2lEx&q=${searchQuery}&limit=20&offset=0`
    )
      .then((resp) => resp.json())
      .then((resp) => {
        const resultArray: string[] = [];
        resp.data.map((data: any) => resultArray.push(data.embed_url));
